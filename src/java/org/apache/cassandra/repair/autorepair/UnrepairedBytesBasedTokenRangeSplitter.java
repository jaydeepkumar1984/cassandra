...
public class UnrepairedBytesBasedTokenRangeSplitter implements IAutoRepairTokenRangeSplitter
{
        ...
        for (Range<Token> tokenRange : tokenRanges)
        {
            ...
            try (Refs<SSTableReader> refs = getSSTableReaderRefs(keyspaceName, tableName, tokenRange))
            {
                for (SSTableReader reader : refs)
                {
                    ...
                    if (!reader.isRepaired())
                    {
                        ...
                        try (BigTableScanner scanner = (BigTableScanner) BigTableScanner.getScanner(reader, Collections.singleton(tokenRange)))
                        {
                            ...
                            logger.info("Calculations for {}.{} {}: sstableSize={}, rangeBytesInSSTable={}, startPosition={}, endPosition={}, ratio={}",
                                        keyspaceName, tableName, reader.descriptor.baseFilename(),
                    ...
                    {
                        logger.info("Skipping over {}.{} {} ({}) because it is repaired", keyspaceName, tableName, reader.descriptor.baseFilename(), FileUtils.stringifyFileSize(reader.bytesOnDisk()));
                    }
                }
            }
            ...
            if (totalBytesInUnrepairedSSTables > 0L)
            {
                ...
                if (approximateUnrepairedBytesForRange < subrangeBytes)
                {
                    ...
                    RepairAssignment assignment = new BytesBasedRepairAssignment(tokenRange, keyspaceName, Collections.singleton(tableName), approximateUnrepairedBytesForRange);
                ...
                else
                {
                    ...
                    {
                        RepairAssignment assignment = new BytesBasedRepairAssignment(splitRange, keyspaceName, Collections.singleton(tableName), approximateBytesPerSplit);
            ...
            else
            {
                ...
                if (memtableSize > 0L)
                {
                    ...
                    RepairAssignment assignment = new BytesBasedRepairAssignment(tokenRange, keyspaceName, Collections.singleton(tableName), memtableSize);
    ...
    public static class BytesBasedRepairAssignment extends RepairAssignment
    {
        ...
        public BytesBasedRepairAssignment(Range<Token> tokenRange, String keyspaceName, Set<String> tableNames, long approximateBytes)
        {
